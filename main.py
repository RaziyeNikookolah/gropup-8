...
def get_items(item_per_page: int, page_num: int):
    ...
    totalpage=len(item_list)//item_per_page+(1 if len(item_list) % item_per_page else 0)
...
def read_item(request: Request):
    return template.TemplateResponse("index.html", {"request": request , "items":item_list})
